...
export function EditPlans() {
  const { id: clientId, planId } = useParams<PlansParams>()
            ...
            return (
              ...
              >
                      ...
                      >
                        {plan.features.map((feat) => (
                          <List.Item
                            ...
                            gap="3"
                          >
                            <Icon fontSize="2xl" color="purple.600">
                              <span>
                                <MdCheckCircle />
                              </span>
                            </Icon>
                ...
                <Card.Footer justifyContent="center">
                  <Button size="xs">
                    <Text
                      color={isSelectedPlan ? 'purple.200' : 'inherit'}
                      display="flex"
                      alignItems="center"
                    >
                      {isSelectedPlan ? 'Selecionado' : 'Selecione'}
                      {isSelectedPlan && (
                        <Icon marginLeft="2" fontSize="lg" color="purple.200">
                          <span>
                            <MdCheckCircle />
                          </span>
                        </Icon>
      ...
      <SectionActions justifyContent="space-between">
        <Skeleton loading={isPlansLoading}>
          <Button variant="outline" asChild>
            <Link to={`/clients/${clientId}/details`}>Cancelar</Link>
          </Button>
        </Skeleton>
        <Skeleton loading={isPlansLoading}>
          <Button disabled={!selectedPlan} asChild>
            <Link to={`/clients/${clientId}/modules`}>Continuar</Link>
          </Button>
        </Skeleton>
      </SectionActions>
    </SectionRoot>
